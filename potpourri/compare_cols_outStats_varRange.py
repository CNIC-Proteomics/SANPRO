...
parser = argparse.ArgumentParser(
    description='Getting the points that are paralleled to X=Y generating a scatter plots',
    epilog='''Examples:
...
def main(args):
    ...
    logging.info(f"getting the the points that are parallel to the X=Y line...")
    # get the variance between points
    # data = data.dropna()
    data['var'] = data.var(axis=1)
    # get the points with a variance greater than...
    data['var_higher'] = data['var'] >= .1
    # save table
    of = os.path.splitext(ofile)[0]+'.tsv'
    data.to_csv(of, sep="\t")
    
    
    logging.info(f"creating the {t_graph} plot...")
    data = data[data['var_higher']]
